...
/* Try to test ADC.*/
    ...
    while(1){
      ...
      USART_puts(USART3, "PC0 : \n\r");
      USART_putd(USART3, tmp1);
      USART_puts(USART3, "\n\r");
...
void DMA_configure(){
        ...
        DMA_InitStructure.DMA_Memory0BaseAddr =(unsigned int) & ADC_ConvertedValue;
...
void adc_configure(){
  ...
  ADC_init_structure.ADC_Resolution = ADC_Resolution_10b;//Input voltage is converted into a 10-bit number whose maximum value is 1023
  ...
  ADC_init_structure.ADC_ScanConvMode = ENABLE;//No scan (only one channel)
